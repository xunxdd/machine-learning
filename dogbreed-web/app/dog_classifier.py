...
def Xception_predict_breed(img_path):
    ...
    bottleneck_feature = app.xception_model.predict(preprocess_input(tensor))
    # obtain predicted vector
    predicted_vector = app.model.predict(bottleneck_feature)

    # return dog breed that is predicted by the model
    return dog_names[np.argmax(predicted_vector)]

def get_label(file_name):
    #result = Xception_predict_breed(os.path.join(app.config['UPLOAD_FOLDER'], file_name))
    return Xception_predict_breed(file_name)